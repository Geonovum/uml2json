...
.13+| \<<property>> | inlineOrByReference | xref:http://www.opengis.net/spec/uml2json/1.0/req/by-reference-basic[style=id%] | see <<jsonschema_byreference_req_basic>>
...
#TBD# Should we add a section to explain how JSON Schema files can be used together with actual JSON data, i.e. instance documents? JSON Schema only recommends the use of links with rel=describedBy (see http://json-schema.org/draft/2020-12/json-schema-core.html#name-associating-instances-and-s). Such a link can be added to the http header of a server response that contains the actual JSON (instance) data. Specific formats may encode such links in the JSON data itself. Also see https://ietf-wg-httpapi.github.io/mediatypes/draft-ietf-httpapi-rest-api-mediatypes.html and the "schema" parameter defined there. Some tools (e.g., the oXygen editor) use a "$schema" member in JSON (instance) data to reference the applicable JSON Schema. However, that is a tool-specific approach. In principle, tool-specific approaches are allowed. However, the choice of re-using the "$schema" keyword in that way is problematic, since it does not reflect the intent of "$schema" as defined by http://json-schema.org/draft/2020-12/json-schema-core.html#name-the-schema-keyword.
...
NOTE: Additional requirements can override this behavior, by omitting certain UML properties or by mapping certain UML properties to first-level members of the resulting JSON object. Examples are the <<jsonschema_req_geojson_formats_primary_geometry,primary geometry property>> as well as the <<jsonschema_req_geojson_formats_identifier,identifier property>>.
...
* xref:http://www.opengis.net/spec/uml2json/1.0/req/by-reference-uri[style=id%] - by-reference encoding of property values using a plain URI (reference, i.e., and absolute or relative URI)
