...
NOTE: For backwards-compatibility to UML 1 schemas (that comply with earlier versions of <<ref_iso19103>>) stereotype «type» can be used instead of stereotype «interface» (the model element for stereotype «type» is Class).
...
include::../requirements/jsonfg/REQ006.adoc[]

NOTE: This allows the primary geometry value to be encoded in the top-level "place" member. There can be different scenarios where that may be relevant. For further details, see <<ref_jsonfg>>, section "7.5 Geometry".
