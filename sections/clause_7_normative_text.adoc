...
[#table_umlprofile_stereotypes,reftext='{table-caption} {counter:table-num}']
.Stereotypes relevant for JSON Schema conversion rules
...
|union |Class |A structured data type without identity where exactly one of the properties of the type is present in an instance. <<ref_iso19103>> #TBD#: This is from the INSPIRE UML profile. A different use would be as type discriminator union, which is what <<ref_iso19103>> defines.
...
|identifier |Attribute (not of an enumeration or code list) |#TODO#
|enum |Attribute of an enumeration |#TODO#

|===

NOTE: Communities may use aliases for the stereotypes listed above.

NOTE: Some conceptual schemas and application schemas do not make use of the stereotypes \<<property>> and \<<enum>>, but still attach certain tagged values to according properties in their UML model. That approach is supported by some UML modeling tools, even though tagged values typically belong to a certain stereotype. For the purposes of this specification, the stereotypes \<<property>> and \<<enum>> are assumed to be applied in schemas that shall be converted from UML to JSON Schema. Nevertheless, it is allowed for schemas to omit the stereotypes, and just use the associated tagged values (see below) on according model elements. This kind of use implies the presence of an ad-hoc stereotype, which is considered to represent the \<<property>> or \<<enum>> stereotypes.

Tagged values relevant for the JSON Schema conversion rules, per stereotype:

#TODO JE# Remove stereotypes from this list, if at the end no specific tagged values are defined for them. Furthermore, omit stereotype \<<enum>> in the UML profile if no tag is defined for it.

* \<<applicationSchema>> and \<<schema>>:
** jsonId - #to be documented#
** jsonDocument - #to be documented#
* \<<featureType>>
* \<<interface>> and \<<type>>
* \<<dataType>>
* \<<union>>
* \<<enumeration>>
** literalEncodingType -  #to be documented#
* \<<codeList>>
** literalEncodingType - #to be documented#
* \<<property>>
** defaultGeometry - #to be documented#
** inlineOrByReference
** jsonFormat - #to be documented#; for basic types
** jsonPattern - #to be documented#; for basic types
** length, maxLength, size - #to be documented#; for basic types ... we should choose a single TV
** rangeMinimum - #to be documented#; for basic types
** rangeMaximum - #to be documented#; for basic types
** sequenceNumber
* \<<voidable>>
* \<<enum>> 

#TBD# ... do we need to define tagged value _jsonEncodingRule_? ... Looks like an implementation detail to me.
...
* Introduction to the content of this specification: three encoding rules, plus a couple of additional requirements classes for special model constructs (briefly introduce them).
* Tabular overview of requirements classes ... maybe with short comment / overview.
* Diagram of requirements classes that shows the dependencies between them
...
The conversion of the class properties is defined in the <<jsonschema_schemaconversionrules_properties>> section. General type conversion rules, such as those documented in the <<jsonschema_req_core_types_classname>> section, may apply.
...
#TBD# If we do not want to require the omission of the identifier property or properties, instead leaving that up to communities
