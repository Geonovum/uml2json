...
image::figures/uml_for_definitions_schema_example.png[align="center"]

[#img_json_schema_structure,reftext='{figure-caption} {counter:figure-num}']
.Deriving JSON Schemas from an application schema
image::figures/definitions_schemas.png[align="center"]
...
[#table_externaltypes_iso19103,reftext='{table-caption} {counter:table-num}']
.JSON Schema implementation of types defined by ISO 19103
...
#Telcon 17.11.2022:# Be careful about use of format, because validators can ignore it. Patterns may be more effective / useful. Add explanatory note.

#TBD# Should we omit the measure types, since the encoding as just real might be a bit restrictive (omitting any uom information). If not, we should probably list them all. -> Telcon 17.11.2022: Depends upon the community. We probably need to support provision of a uom. Since Measure is such an important, basic type defined in ISO 19103, we define a JSON Schema for Measure (a JSON object with value and uom identifier). Add a note that if ISO defined a JSON Schema for 19103 Measure, that could be used.

#TODO# Define these tables per JSON encoding rule, as necessary

[#table_externaltypes_iso19107_geojson,reftext='{table-caption} {counter:table-num}']
.JSON Schema implementation of types defined by ISO 19107 - using GeoJSON schemas
[width="90%",options="header"]
|===
...
#TODO# What are the intended final file locations for the JSON-FG JSON Schema files- the URLs with "beta." removed? -> Telcon 17.11.2022: Yes, that is the idea. We have to nail down the version of ISO 19107 in the normative references section. The v2003 should be supported. Add a note that application schemas may use a newer version of ISO 19107, and that this document here simply does not define mapping tables for versions other than the 2003 one.

[#table_externaltypes_iso19107_jsonfg,reftext='{table-caption} {counter:table-num}']
.JSON Schema implementation of types defined by ISO 19107 - using JSON-FG schema definitions
[width="90%",options="header"]
|===
| UML class | JSON Schema reference
|GM_Curve |https://beta.schemas.opengis.net/json-fg/geometry-objects.json#/$defs/LineString
|GM_MultiCurve |https://beta.schemas.opengis.net/json-fg/geometry-objects.json#/$defs/MultiLineString.json
|GM_MultiPoint |https://beta.schemas.opengis.net/json-fg/geometry-objects.json#/$defs/MultiPoint.json
|GM_MultiSolid |https://beta.schemas.opengis.net/json-fg/geometry-objects.json#/$defs/MultiPolyhedron
|GM_MultiSurface |https://beta.schemas.opengis.net/json-fg/geometry-objects.json#/$defs/MultiPolygon.json
|GM_Object |https://beta.schemas.opengis.net/json-fg/geometry.json
|GM_Point |https://beta.schemas.opengis.net/json-fg/geometry-objects.json#/$defs/Point.json
|GM_Solid |https://beta.schemas.opengis.net/json-fg/geometry-objects.json#/$defs/Polyhedron
|GM_Surface |https://beta.schemas.opengis.net/json-fg/geometry-objects.json#/$defs/Polygon.json
|===
...
#TBD# This is primarily applicable to GeoJSON-based encoding rules, i.e. the GeoJSON and JSON-FG encoding rules. Both GeoJSON and JSON-FG define their own JSON Schema for features. -> ii recommendation: requirements defining a common base schema should only be defined for GeoJSON and JSON-FG, and only for \<<featureType>>s ... though maybe also for object types? -> Telcon 17.11.2022: common base schema for \<<featureType>>s in GeoJSON and JSON-FG ... add a NOTE that object types may need to be converted / promoted on-the-fly to feature types if they should be encoded as GeoJSON features.

#TBD# We may need a common base schema for plain encoding rule as well, iff there is not means to add an id member there to types with identity.
...
#TBD# All of the following would go too far. -> ii recommendation: stop here and simply refer to the UGAS-2020 ER (i.e., remove the following NOTE and refer to the UGAS-2020 ER, for more details about a potential conversion approach) -> Telcon 17.11.2022: Agreed, we stop here and reference the ER
...
image::figures/Specialization_example.png[align="center"]
...
----
{
  ...
  "anyOf": [
    {"$ref": "#/$defs/Enumeration1"},
    {"$ref": "#/$defs/Enumeration2"}
  ]
}
----
...
#TBD# The situation for code lists is similar to that of unions. We have multiple conversion behaviors for converting attributes with a code list value type: the type is either a simple type (e.g. string or number), a URI, or a link object. -> telcon 24.11.2022: create separate requirements classes for all options; have a common requirements class for them that adds the requirement to encode tagged value 'codeList' (also defined by ISO 19103) as property 'codeList' in the JSON Schema definition of a code list type
...
NOTE: The URL to the JSON Schema of the "Link" object (as shown in <<example_jsonschema_link_object,this code block>>) can be configured using the ShapeChange JSON Schema target parameter _linkObjectUri_.

[[example_jsonschema_link_object]]
...
#TBD# Not sure if we actually need to support basic types. Is that something that occurs in application schemas that need to be encoded by Geonovum? -> telcon 24.11.2022: keep basic types, use maxLength only, differentiate cases for minimum inclusive and exclusive - and use the facet names that XML schema (min/maxInclusive/Exclusive)
...
#TBD# Do we need multiple options for the byReference-encoding? Right now, we have both a URI option as well as the option of using a link object. Interestingly, there is an early activity of standardizing or harmonizing references in JSON: https://github.com/json-schema-org/referencing. However, that activity is still ongoing and right now there are competing proposals. -> telcon 24.11.2022: allow both options as separate requirements classes (link object is most "webby" ... maybe link to discussion in JSON-FG Annex about linking); add overview diagram with groups of related requirements classes; e.g. here for req classes for encoding associations (which simple application schemas may not even need)
...
image::figures/Voidable_example.png[align="center"]
...
#TBD# Shall we encode derived properties? If so, the NOTE should become a separate section. -> telcon 24.11.2022: keep derived properties, make it a separate section
...
image::figures/GeoJSON_example.png[align="center"]
...
This chapter defines requirements classes for the encoding of application schemas in UML as JSON Schema, and likewise for the encoding of application schema data as JSON data. 


[[jsonschema_uml_profile]]
=== UML profile

// summary of the relevant UML profile - stereotypes and their tagged values

#TBD# Move to an annex, to clause 6 (informative)? The conventions chapter does not seem to be the right place ...
...
|schema |Package |This stereotype is typically used in abstract schemas defined by ISO TC 211. For further details on abstract schemas, see <<ref_iso19103>>, chapter 6.2 and figure 4. An abstract schema and an application schema are both conceptual schemas, but they are on different levels of abstraction.
|featureType |Class |A feature type as defined by <<ref_iso19109>>.
|interface - for backwards-compatibility to UML 1 schemas (that comply with earlier versions of <<ref_iso19103>>) also: type |Class |An abstract classifier with operations, attributes and associations, which can only inherit from or be inherited by other interfaces. Other classifiers may realize an interface by implementing its operations and supporting its attributes and associations (at least through derivation). Source: <<ref_iso19103>> This stereotype is typically used in conceptual schemas from ISO TC 211. It should not be used in application schemas, as these are on a different conceptual level than classifiers with this stereotype.
|dataType |Class |A set of properties that lack identity (independent existence and the possibility of side effects). A data type is a classifier with no operations, whose primary purpose is to hold information. Source: <<ref_iso19103>>
...
|enumeration |Class |A fixed list of valid identifiers of named literal values. Attributes of an enumerated type may only take values from this list. Source: <<ref_iso19103>>
...
|#voidable# |Property (attribute [not of an enumeration or code list] or association role) |#TODO#
...
.2+| \<<applicationSchema>> and \<<schema>> | jsonDocument | xref:http://www.opengis.net/spec/uml2json/1.0/req/core[style=id%] | see <<jsonschema_req_core_definitionsschema>>
| jsonId  | xref:http://www.opengis.net/spec/uml2json/1.0/req/core[style=id%] | see <<jsonschema_req_core_definitionsschema>>
...
.12+| \<<property>> | inlineOrByReference | xref:http://www.opengis.net/spec/uml2json/1.0/req/core[style=id%] | see <<jsonschema_byreference_req_basic>> and <<jsonschema_req_core_properties_valuetype>>
| jsonPrimaryGeometry | xref:http://www.opengis.net/spec/uml2json/1.0/req/geojson-formats[style=id%] | see <<jsonschema_req_geojson_formats_primary_geometry>>
| jsonPrimaryInstant | xref:http://www.opengis.net/spec/uml2json/1.0/req/jsonfg[style=id%] .2+| see <<jsonschema_req_jsonfg_primary_time>>
| jsonPrimaryInterval | xref:http://www.opengis.net/spec/uml2json/1.0/req/jsonfg[style=id%]  
| jsonPrimaryPlace | xref:http://www.opengis.net/spec/uml2json/1.0/req/jsonfg[style=id%] | see <<jsonschema_req_jsonfg_primary_place>>
...
NOTE: The "$id" of the definitions schema is not included in other examples within this chapter, because declaring an absolute, non-existent URL in these examples often prevents JSON Pointers from these examples from working when testing the examples, for instance on https://www.jsonschemavalidator.net/.
...
Whenever an external type is used, its JSON Schema definition is needed. Either an external type is implemented as one of the simple JSON value types (e.g., string - maybe with a certain format or pattern), or it is defined by a particular JSON Schema. In case of a JSON Schema, the URL of that schema needs to be known during the encoding process. If the schema is a definitions schema, then the URL needs to be augmented with a fragment identifier that includes a JSON Pointer or an anchor reference within the schema. 
...
|Boolean |boolean |
|CharacterString |string |
|Date |string a|format=date, pattern= 
[source,options="unnumbered"]
----
^\d{4}-\d{2}-\d{2}$
----
|DateTime |string a|format=date-time, pattern= 
[source,options="unnumbered"]
----
...
|Number |number |
|Real |number |
|Time |string a|format=time, pattern= 
[source,options="unnumbered"]
----
...
^(([^:/?#]+):)?(\/\/([^/?#]*))?([^?#]*)(\?([^#]*))?(#(.*))?$
----
which is defined (without the last '$') in https://www.rfc-editor.org/rfc/rfc3986.html#appendix-A ? RFC 3986 is referenced from http://json-schema.org/draft/2020-12/json-schema-validation.html#section-7.3.5[JSON Schema] for the 'uri' format. Or omit the pattern for URI?
...
#TBD# Would it be better to define the patterns to use for specific JSON Schema formats shown in <<table_req_core_iso19103_primitive_types>> in this recommendation? That way, the specification would still list them, and implementations can choose to use them - or to omit them.

include::../requirements/core/REQ004.adoc[]

[[jsonschema_for_iso19103_measure_types]]
.JSON Schema for ISO 19103 measure types
[source,json,linenumbers]
----
{
  "type" : "object",
...
NOTE: Keep in mind that the use of an $anchor value in $ref references can depend upon the media type with which the referenced JSON schema is published, as is explained in more detail <<jsonschema_req_core_definitionsschema_media_types,here>>.
...
[source,json,linenumbers,options="unnumbered"]
----
{
  "propertyA": 2,
  "propertyB": "x"
...
No specific model transformation requirements and recommendations are defined in this document. Examples of model transformations are given in <<ref_ugas2020>> and in the https://github.com/INSPIRE-MIF/2017.2[GitHub repository of the INSPIRE action 2017.2 on alternative encodings].
...
include::../requirements/plain/REQ001.adoc[]
...
#TBD# What about solids - should we refer to the JSON-FG geometry then? ... and what about other geometry types, then, such as GM_Curve - should that always be a GeoJSON LineString, or should we also refer to the JSON-FG geometry in such cases? Do we need to reference some specific section of GeoJSON or JSON-FG in the dependencies of the requirements class? ... probably yes...
...
[[jsonschema_req_geojson_formats_iso19107_types]]
...
.JSON Schema implementation of types defined by ISO 19107, for the "geometry" member in GeoJSON-based encoding rules
[width="90%",options="header"]
|===
| UML class | JSON Schema reference
|GM_Curve |https://geojson.org/schema/LineString.json
...
#TBD# Should we suggest the JSON-FG geometries for all non-primary-geometry cases? Likely depends on application requirements regarding dimension: if 3D, use JSON-FG geometries, otherwise use GeoJSON geometries. CRS choice can only be encoded with JSON-FG feature or feature collection. Also, we should not define such a requirement here. That should be left to the actual GeoJSON and JSON-FG encoding requirements classes. The former would likely only use GeoJSON geometry types for non-primary-geometry properties, while the latter only uses JSON-FG geometry types for such properties.
...
include::../requirements/geojson-formats/REQ003.adoc[]

NOTE: Additional requirements can override this behavior, by omitting certain UML properties or by mapping ceratin UML properties to first-level members of the resulting JSON object. Examples are the <<jsonschema_req_geojson_formats_primary_geometry,primary geometry property>> as well as the <<jsonschema_req_geojson_formats_identifier,identifier property>>.
...
[[jsonschema_req_geojson_formats_primary_geometry]]
==== Primary geometry

include::../requirements/geojson-formats/REQ004.adoc[]

NOTE: UML properties of other kinds of classes - object types, data types, and unions - are not considered by this requirements. Object types are not encoded as GeoJSON features. Data types and unions may be used by other classes, which prevents a general exclusive mapping to the GeoJSON top-level "geometry" member. Only a direct property of a \<<featureType>> can be mapped in this way.

[[img_req_geojson_formats_primary_geometry_example]]
...
[[example_req_geojson_formats_primary_geometry]]
...
NOTE: The following sections refer to JSON-FG JSON Schema files via URLs that contain the string "beta." Once the JSON-FG standard and its JSON Schema definitions have been approved by OGC, these JSON Schema files are going to be available at the mentioned URLs, but without the "beta." part. For example, the geometry.json schema file is going to be available at https://schemas.opengis.net/json-fg/geometry.json.

#TBD# Add a requirement for implementation of non-primary-geometry properties using the GeoJSON geometry definitions?
...
#TBD# Add a requirement for implementation of non-primary-geometry properties using the JSON-FG geometry definitions?
...
[[jsonschema_req_jsonfg_primary_place]]
==== Primary place

include::../requirements/jsonfg/REQ003.adoc[]

NOTE: UML properties of other kinds of classes - object types, data types, and unions - are not considered by this requirements. Object types are not encoded as JSON-FG features. Data types and unions may be used by other classes, which prevents a general exclusive mapping to the JSON-FG top-level "place" member. Only a direct property of a \<<featureType>> can be mapped in this way.

[[img_req_jsonfg_primary_place_example]]
.Example of a feature type with an attribute designated as primary place
image::figures/example_primary_place.png[align="center"]
...
[[jsonschema_req_jsonfg_primary_time]]
==== Primary time
...
include::../requirements/jsonfg/REQ005.adoc[]

NOTE: UML properties of other kinds of classes - object types, data types, and unions - are not considered by this requirements. Object types are not encoded as JSON-FG features. Data types and unions may be used by other classes, which prevents a general exclusive mapping to the JSON-FG top-level "time" member. Only a direct property of a \<<featureType>> can be mapped in this way.

////
This example is probably not that helpful, since the only other property is mapped to the GeoJSON "geometry" member and no other properties are defined.
...
Requirements class xref:http://www.opengis.net/spec/uml2json/1.0/req/core[style=id%] specifies an <<jsonschema_req_core_properties_valuetype,inline encoding of property values>>. In case that the value type of a UML property is a type with identity (that is not implemented as a simple JSON Schema type), it can be preferable and maybe even necessary to encode the value by reference. In other cases, both options should be offered. That is similar to what the GML Application Schema encoding rules support (for further details, see OGC 07-036r1, Annex E, section E.2.4.11).
...
* xref:http://www.opengis.net/spec/uml2json/1.0/req/by-reference-uri[style=id%] - by-reference encoding of property values using a plain URI
...
NOTE: The default value for tag _inlineOrByReference_ is different to GML. The value _byReference_ has been chosen as default, in order to reduce the degrees of freedom and to reduce the schema complexity.


[[jsonschema_byreference_encoding_req_uri]]
==== Requirements class: by-reference encoding of property values using a plain URI
...
#TBD# Should we really require the use of format=uri? ... I've tested the above example with #d024i42sl as value for owner on https://www.jsonschemavalidator.net/, and it did not validate. While that may just be due to the implementation of that page, it represents an issue that we should consider. Whenever an in-document reference is supposed to be put as content of a reference field, the question is if the format=uri may cause validation to fail. ... and I know that in-document references are not (yet) defined in JSON, so a community-specific mechanism would have to be defined. Note that the link object schema at https://schemas.opengis.net/ogcapi/common/part1/1.0/openapi/schemas/link.json also does not define a format for "href". If we decided to avoid format=uri here, and just use type=string, then the note in the overview section (about foreign key use with simple string or number) could be removed.
...
#TBD# Not sure about the value of "rel". Unfortunately, that is a required property of a link object. There are two registries to consult: http://www.opengis.net/def/rel and https://www.iana.org/assignments/link-relations/link-relations.xhtml. In addition, according to the OGC API Common User Guide, one can define arbitrary URIs as values - which would be seen as extensions to the registered link relations. ... just "owner" is wrong, then, I guess. Would https://defs.opengis.net/vocprez/object?uri=http%3A//www.opengis.net/def/rel/iana/1.0/item[item] be better? https://html.spec.whatwg.org/multipage/links.html#link-type-alternate[alternate] does not appear to be appropriate, either.
...
The result of applying this encoding to the unions from <<img_unions_req_type_discriminator_union_example>> is shown in <<example_unions_req_type_discriminator_jsonschema,the following source block>>.
...
#TBD# Do we have or need a dependency on OGC API Common, if we use the link object schema defined there? Is there a suitable requirements class in OGC API Common? Apparently, part 1 of OGC API Common has recently been published (see https://opengeospatial.github.io/ogcna-auto-review/19-072.html). There is a GitHub repository, which seems to have up-to-date information: https://github.com/opengeospatial/ogcapi-common. There is also a user guide: https://portal.ogc.org/files/?artifact_id=103046&version=1. It looks like there are no specific requirements in OGC API Common regarding the use of link objects. There are some recommendations regarding links and link relations (the user guide also has a chapter on link relations). So, I think we can get away with just referencing the JSON Schema of a link object, and not having a dependency on any of the requirements classes defined by OGC API Common - Part 1.
...
NOTE: The requirements class xref:http://www.opengis.net/spec/uml2json/1.0/req/entitytype[style=id%] is applicable in <<jsonschema_req_plain,plain>> and <<jsonschema_req_geojson,GeoJSON>> JSON encodings. The <<jsonschema_req_jsonfg,JSON-FG>> encoding already provides the "featureType" member, which plays a similar role - at least for JSON objects representing features. Adding an "entityType" member in such an encoding would duplicate the already existing typing mechanism there.

#TBD# For JSON-FG, the "featureType" member is not defined for data types. That is a bit of a mismatch, considering that inheritance hierarchies for complex data types are possible in application schemas, and identifying the type of a data type value in actual JSON data could thus be desired by applications.
